...
- **`0.5.1` 与 `0.5.2` 的内容与 `0.5.0` 完全相同。** 这两个号是发布端的噪音：第一次 `npm publish` 返回的是 HTTP 202，版本要几分钟后才出现在 registry 上，被误当成失败又发了两遍。三个版本都在线上、代码逐字节一致（12 个文件），`latest` 指向 `0.5.2`。装哪个都行；想按标签点名就固定 `0.5.2`。
