...
- **逐条检查插件依赖的宿主接口** — 本插件会重新发出宿主的设计令牌、把毛玻璃挂在宿主的界面元素上、并读取宿主的会话与模型服务；这些每一条都是与某个 dsh 版本之间的约定，而任何一条被改名都不会报错。自检会逐条给出：是否仍然成立、失效时界面上会是什么样子、依赖的那个字面量、本应由哪个宿主文件定义、以及本插件哪个文件在读取它。
- **三种状态，如实报告** — *正常*（宿主仍然提供）、*未观察*（当前无法从界面上看到：设置弹窗打开时对话页不在屏幕上，没开过文件时右侧栏也不在 DOM 里）、*失效*（界面在屏幕上，但接口已经没了）。只有第三种算故障。
- **复制诊断报告** — 一键把两侧结果（实时探测 + 对安装目录的离线扫描）合并成一份 markdown，直接贴进 issue 即可。
- **离线等价物** — `pnpm scan` 用同一张约定表检查已安装的包，不需要浏览器；一旦某个目标消失就以非零码退出。升级 dsh 后先跑它。
...
| **宿主自检** | — | 逐条检查本插件依赖的宿主接口在当前 dsh 上是否仍然成立，并给出可复制的诊断报告 |
...
插件与 dsh 之间只通过一张明确列出的接口清单通信（见 [`src/host-contracts.ts`](src/host-contracts.ts)）。升级 dsh 之后，先查它，再怀疑别的地方：

```sh
pnpm run bundle
pnpm run scan    # 离线：某个约定目标从已安装的 dsh 里消失就以非零码退出
pnpm test        # 约定表、实时探测逻辑、界面样式表、版本下限
```

`pnpm scan` 支持 `--host <dir>`（含 `@deepseek-ai/dsh-app-boot` 的 `node_modules` 目录）与 `--json`。同一套检查也在应用内运行：设置 → **宿主自检**，对着真实界面跑。
