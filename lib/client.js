window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			exportCardDesc: "将主题色、透明度、模糊与壁纸打包为 dsh-background-by-model-theme.json 文件",
		...
		const en = {
			...
			exportCardDesc: "Bundle color, opacity, blur and wallpaper into dsh-background-by-model-theme.json",
		...
		const VIDEO_SERVE_URL = "/dsh-background-by-model/video";
		/** HTTP route new videos are POSTed to as raw bytes (see uploadVideo). */
		const VIDEO_UPLOAD_URL = "/dsh-background-by-model/video/upload";
  ...
  `;
		...
		*  callback returns. */
		...
		function reassertScheme() {
			const [, , l] = rColor();
			if (rBgDark() ?? l < .55) document.body.setAttribute("data-ds-dark-theme", "dsh-background-by-model");
			else document.body.removeAttribute("data-ds-dark-theme");
			applyCustomTokens(rOps());
		}
