window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const en = {
			...
			uiOpacitySide: "Left panel",
			uiOpacityCard: "Cards & menus",
		...
		*/
		...
		function clamp$1(n, lo, hi, def) {
			return typeof n === "number" && isFinite(n) ? Math.min(hi, Math.max(lo, n)) : def;
		}
		const clamp01 = (n, def) => clamp$1(n, 0, 1, def);
		function normalizeBgState(s) {
			const v = s ?? {};
			return {
				zoom: clamp$1(v.zoom, .1, 10, 1),
		...
		function normalizeHsl(raw) {
			...
			return [
				clamp$1(raw[0], 0, 360, 220),
				clamp$1(raw[1], 0, 1, .55),
				clamp$1(raw[2], 0, 1, .25)
			];
		}
		...
		function normalizeRule(raw) {
			...
			const mode = BG_MODES$1.includes(r.bgMode) ? r.bgMode : "fit";
			return {
				id,
				slot,
		...
		function normalizeConfig(raw) {
			...
			const bl = r.blurs ?? {};
			const blurs = {};
			for (const k of PART_BLUR_KEYS) blurs[k] = clamp$1(bl[k], 0, 60, DEFAULT_PART_BLURS[k]);
			const opacities = {};
		...
		*  the data URL when no object URL can be built. */
		...
		function rOps() {
			...
			const out = {};
			for (const k of PART_OPACITY_KEYS) out[k] = clamp01(o[k], DEFAULT_PART_OPACITIES[k]);
			return out;
		}
		function rBlurs() {
			const b = cfg.blurs ?? {};
			const out = {};
			for (const k of PART_BLUR_KEYS) out[k] = clamp$1(b[k], 0, 60, DEFAULT_PART_BLURS[k]);
			return out;
		}
		function rSop() {
			return clamp01(cfg.settingsOpacity, 1);
		}
		function rChatTextOpacity() {
			return clamp01(cfg.chatTextOpacity, 0);
		}
		function rTrajectoryOpacity() {
			return clamp01(cfg.trajectoryOpacity, 1);
		}
		/** Own opacity of the file-preview panel; while unowned it IS the main background's. */
		function rRightbarOpacity() {
		...
		*/
		async function readHostScan(lang) {
		...
		*/
		...
		function buildTokens(hue, sat, lit) {
			...
			if (dark) {
				const menuFill = hsl(h(0), s(0), l(.08));
				return {
					colorScheme: "dark",
					tokens: {
			...
			const menuFill = hsl(h(0), s(-.12), l(.15));
			return {
				colorScheme: "light",
				tokens: {
		...
		const RIGHTBAR_STYLE_RULE = "div[data-sidebar-right-panel]:not(:has([data-dockkit-content],[data-dockkit-empty]))[data-sidebar-right-open],div[data-sidebar-right-panel] [data-dockkit-content]:not([data-dockkit-float]),div[data-sidebar-right-panel] [data-dockkit-empty]{background:var(--dsh-any-bg-rightbar,var(--dsw-alias-bg-base))!important}div[data-sidebar-right-panel][data-sidebar-right-open]{-webkit-backdrop-filter:var(--dsh-any-blur-rightbar,none);backdrop-filter:var(--dsh-any-blur-rightbar,none)}";
		/** Paint the file-preview panel's surface from its own slider. */
		function applyRightbarOverrides(op) {
			const root = document.documentElement;
			if (!rHasColor()) {
				root.style.removeProperty("--dsh-any-bg-rightbar");
				return;
			}
			const [h, s, l] = rColor();
			const base = genTokens(h, s, l).tokens["--dsw-alias-bg-base"];
			if (base !== void 0) root.style.setProperty("--dsh-any-bg-rightbar", toRgba(base, op));
		}
		/** Extra backdrop frost for the file-preview panel (0 = no frost of its own). */
		function applyRightbarBlur(px) {
		...
		*/
		function resolveMainSessionId(uiCurrent, snapshot) {
			...
			if (byId !== void 0 && byId !== null) {
				for (const row of Object.values(byId)) if ((row?.retainedBy?.mainView ?? 0) > 0) {
		...
		function mainSessionId(ctx, list) {
			let uiCurrent;
			try {
				uiCurrent = ctx.get("uiSession")?.current?.getSnapshot?.();
			} catch {}
		...
		function browserEnv(ctx) {
			return {
				...
				computedValue: (token, on) => {
					...
					const host = on ?? document.documentElement;
		...
		*/
		function ruleMentioned(env, literal) {
			for (const sheet of env.styleSheets()) {
				if (sheet.ownerNode?.dataset?.plugin !== void 0) continue;
				let rules = null;
				try {
		...
		function evaluate(env, check) {
			switch (check.kind) {
				...
				case "selector": {
					...
					if (check.optionalWhen !== void 0 && !mounted(env, check.optionalWhen)) return {
						status: "skip",
						name,
				...
				case "rule": {
					...
					if (!stylesReadable(env)) return {
						status: "skip",
						name,
						detail: "no stylesheet is readable"
					};
					return ruleMentioned(env, check.match) ? {
						status: "pass",
						name,
						detail: "declared by a host rule"
					} : {
						status: "fail",
						name,
						detail: "no rule mentions it",
						reason: `no stylesheet declares ${check.match}`
					};
				}
				default: return {
					status: "fail",
		...
		*/
		...
		function buildClientReport(input, host) {
...
`;
		...
		const CopyIcon = ({ size, className }) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(Glyph, {
			size,
			className,
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("rect", {
				...
				height: "7.6",
				rx: "1.6"
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", { d: "M10.6 5.4V4.2a1.6 1.6 0 0 0-1.6-1.6H4.2a1.6 1.6 0 0 0-1.6 1.6v4.8a1.6 1.6 0 0 0 1.6 1.6h1.2" })]
		});
		...
		const PreviewIcon = ({ size, className }) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(Glyph, {
			size,
			className,
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("rect", {
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", { d: "M9.4 2.2v11.6" }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", { d: "M11 5.6h1.2M11 8h1.2" })
			]
		});
		...
		function InterfacePage({ p }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
				...
				children: PARTS.map((part, i) => {
					...
					const blurKey = isChat ? "chat" : isTrajectory ? "trajectory" : isRightbar ? "rightbar" : isSettings ? "settings" : opKey;
					const opacity = isChat ? rChatTextOpacity() : isTrajectory ? rTrajectoryOpacity() : isRightbar ? rRightbarOpacity() : isSettings ? rSop() : rOps()[opKey];
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(LiveSlider, {
								...
								onChange: (v) => {
									...
									if (isChat) {
										...
										applyTrajectoryOverrides(op);
										saveConfig();
									} else if (isRightbar) setRightbarOpacity(op);
		...
		*/
		...
		function ModelBgPage({ p, notify }) {
			...
			(0, react.useEffect)(() => {
				if (!touched) setText(store.model);
			}, [store.model, touched]);
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							...
							}), store.model !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "dab-btn",
								onClick: () => {
									setTouched(true);
								...
								children: t("tryoutUseCurrent")
							}) : null]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "dab-hint",
							style: { marginTop: 9 },
							children: probe === "" ? t("tryoutEmpty") : result === null ? t("tryoutNone") : result
						}),
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: `dab-toggle${cfg.autoExtract ? " is-on" : ""}`,
								role: "switch",
								"aria-checked": cfg.autoExtract,
								title: t("autoExtract"),
								onClick: () => p.setAutoExtract(!cfg.autoExtract),
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: "dab-toggle-knob" })
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dab-hint",
								children: t("autoExtract")
							})]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "dab-hint",
							style: { marginBottom: 11 },
							children: t("autoExtractHint")
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dab-rules",
		...
		//#endregion
		//#region src/client/components/pages/HostPage.tsx
		/**
		* The "Host check" page.
		*
		...
		*/
		...
		function HostPage({ p, notify }) {
			...
			const statusWord = (s) => s === "pass" ? t("hostPass") : s === "fail" ? t("hostFail") : s === "skip" ? t("hostNa") : t("hostInfo");
			const row = (r) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
				className: `dab-check dab-check-${r.status}`,
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: `dab-check-ico dab-check-ico-${r.status}`,
					"aria-hidden": "true",
					children: r.status === "pass" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CheckIcon, { size: 12 }) : r.status === "fail" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AlertIcon, { size: 12 }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: "dab-check-dot" })
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "dab-check-body",
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dab-check-head",
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dab-check-label",
								children: r.label
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: `dab-check-status dab-check-status-${r.status}`,
								children: statusWord(r.status)
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", {
								className: "dab-check-target",
								children: r.target
							})
						]
					}), r.status === "fail" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dab-check-reason",
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { children: r.reason ?? r.detail }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: "dab-check-symptom",
								children: r.symptom
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "dab-check-where",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("hostExpectedIn") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: r.sources.map((s) => s.path).join("  ·  ") })]
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "dab-check-where",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("hostUsedBy") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: r.usedBy })]
							})
						]
					}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dab-check-detail",
						title: r.checks.join("\n"),
						children: r.detail
					})]
				})]
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", {
							className: "dab-h1",
							children: t("pageHostCheck")
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "dab-desc",
							children: t("descHostCheck")
						})
					]
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
					className: `dab-card dab-verdict dab-verdict-${verdict} dab-rise`,
					style: { "--d": rise++ },
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "dab-verdict-head",
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: `dab-verdict-ico dab-verdict-ico-${verdict}`,
									"aria-hidden": "true",
									children: verdict === "ok" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CheckIcon, { size: 16 }) : verdict === "unknown" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(LayersIcon, { size: 16 }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AlertIcon, { size: 16 })
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: "dab-verdict-text",
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										className: "dab-verdict-title",
										children: verdict === "ok" ? t("hostAllOk") : verdict === "unknown" ? t("hostUnknown") : verdict === "broken" ? t("hostBroken") : t("hostPartial")
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										className: "dab-verdict-sub",
										children: t("hostSummary").replace("{pass}", String(client?.summary.pass ?? 0)).replace("{fail}", String(client?.summary.fail ?? 0)).replace("{skip}", String(client?.summary.skip ?? 0))
									})]
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: "dab-verdict-actions",
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										type: "button",
										className: "dab-btn",
										disabled: busy,
										onClick: () => void run(),
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(RefreshIcon, { size: 13 }), busy ? t("hostRunning") : t("hostRerun")]
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										type: "button",
										className: "dab-btn dab-btn-primary",
										onClick: () => void copyReport(),
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CopyIcon, { size: 13 }), t("hostCopy")]
									})]
								})
							]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Fact, {
									label: "dsh",
									value: hostVersion !== "" ? hostVersion : t("hostUnreadable"),
									bad: compatible === false
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Fact, {
									label: t("hostPluginFloor"),
									value: `>=${DSH_FLOOR}`
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Fact, {
									label: t("hostPlugin"),
								...
								scanned !== null && scanned.host.node !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Fact, {
									label: "node",
									value: scanned.host.node
								}) : null,
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Fact, {
									label: t("hostDiskScan"),
									value: diskVerdict === "ok" ? `${scanned?.summary.pass ?? 0}/${scanned?.summary.total ?? 0}` : diskVerdict === "unknown" ? t("hostNa") : `${scanned?.summary.fail ?? 0} ${t("hostFail")}`,
									bad: diskVerdict === "broken" || diskVerdict === "partial"
								}),
								client?.host.model !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Fact, {
									label: t("hostModel"),
									value: client.host.model.text !== "" ? client.host.model.text : t("hostModelNone")
								}) : null
							]
						}),
						compatible === false ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "dab-check-note",
							children: t("hostBelowFloor")
						}) : null,
						scanned !== null && scanned.host.note !== "" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "dab-check-note",
							children: scanned.host.note
						}) : null,
						diskFailures(scanned) > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "dab-check-note",
							children: t("hostDiskFailures").replace("{n}", String(diskFailures(scanned)))
						}) : null
					]
				}),
				client === null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("section", {
					className: "dab-card dab-rise",
					style: { "--d": rise },
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dab-check-detail",
						children: t("hostRunning")
					})
				}) : null,
				CONTRACT_GROUPS.map((group) => {
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
						...
						style: { "--d": rise++ },
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "dab-check-group-title",
							children: [lang === "zh" ? group.label.zh : group.label.en, /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								className: "dab-check-group-count",
								children: [
									ok,
									"/",
									groupRows.length
		...
		function diskFailures(report) {
			return report === null ? 0 : report.summary.fail;
		}
		function Fact({ label, value, bad = false }) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: `dab-fact${bad ? " is-bad" : ""}`,
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: "dab-fact-k",
					children: label
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", {
					className: "dab-fact-v",
					title: value,
					children: value
				})]
			});
		...
		*/
		function ThemeSection(props) {
			...
			const pages = [
				...
				{
					label: t("pageHostCheck"),
					Icon: GearIcon,
					node: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HostPage, {
						p: props,
						notify
					})
				}
		...
		*/
		...
		function apply(ctx) {
			...
			*/
			const maybeAutoExtract = async (id) => {
				...
				let hsl = null;
				try {
					hsl = await extractWallpaperColor(url, rule.bgState);
				} catch {
					hsl = null;
				}
				if (hsl === null) return;
			...
			const sectionInject = (actions) => {
				...
				return {
					...
					setRuleImage: (id, dataUrl) => {
						...
						if (dataUrl !== null) maybeAutoExtract(id);
					},
			...
			const hostCheckTimer = window.setTimeout(() => {
				try {
					...
					for (const r of failed) console.warn(`dsh-background-by-model: [${r.id}] ${r.reason ?? r.detail} — read by ${r.usedBy}; expected in ${r.sources.map((s) => s.path).join(", ")}`);
				} catch {}
			}, 2500);
