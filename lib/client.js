window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			statusUnknownHint: "检测不到时一律使用第 1 条可用规则；请确认本机 DSH 版本暴露了模型选择服务",
		...
		const en = {
			...
			statusUnknownHint: "Without a detected model rule 1 is always used; check that this DSH build exposes the model-selection services",
		...
		*/
		function watchModel(ctx, onChange) {
			...
			if (list === void 0 || typeof list.subscribe !== "function" || typeof list.getSnapshot !== "function") return () => void 0;
			...
			const bind = () => {
				...
				if (id === sessionId && directory !== null) {
					emit();
					return;
				}
				...
				directory = null;
				sessionId = id;
				if (id !== null && directories !== void 0 && typeof directories.directoryFor === "function") try {
		...
		*/
		...
		function apply(ctx) {
			...
			const store = defineStore({
				...
				actions: { sync: (d, url, r, rr, model, id, matched) => {
					if (r > d.rev) {
						d.url = url;
						d.rev = r;
					}
